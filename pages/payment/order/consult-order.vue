<template>
  <view class="order-page">
    <!-- 顶部标题栏 -->
    <u-navbar title="咨询订单" :autoBack="true"
      :background="{ backgroundColor: '#f7f8fa' }"></u-navbar>
    <!-- 顶部 tabs -->
    <u-tabs
      :list="tabs"
      @change="changeTab"
    ...
    <scroll-view scroll-y class="order-list">
      ...
      >
        ...
        <view class="order-header">
          ...
          <text class="status green">{{ getStatusText(item.status) }}</text>
        </view>
        ...
        <view class="order-body" @click="toDetail(item)">
          ...
          >
          <view class="desc"
            ><text>到期时间：</text>{{ item.expireTime }}</view
          >
