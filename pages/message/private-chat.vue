<template>
	<view class="chatInterface" @contextmenu.prevent="">
		<view class="scroll-view">
			...
			<view v-else :class="history.allLoaded ? 'history-loaded':''">
				<view>{{ history.allLoaded ? '已经没有更多的历史消息' : '' }}</view>
			</view>
		...
		<view class="action-box" v-if="!videoPlayer.visible && !messageSelector.visible">
			<view class="action-top">
				<view @click="switchAudioKeyboard">
					<image class="more" v-if="audioVisible" src="/static/message/jianpan.png"></image>
					<image class="more" v-else src="/static/message/audio.png"></image>
				</view>
				<view v-if="audioVisible" class="record-input" @touchend.stop="onRecordEnd"
			...
			<view v-if="otherTypesMessagePanelVisible" class="action-bottom">
				...
				<view class="more-icon">
					<image @click="showOrderMessageList()" class="operation-icon" src="/static/message/order.png">
					</image>
					<view class="operation-title">订单</view>
				</view>
				<view class="more-icon">
					<image @click="privateCall()" class="operation-icon" src="/static/message/rtc.png"></image>
...
<script setup>
	...
	function onMessageReceived(message) {
		...
		let friendId = currentUser.id === senderId ? receiverId : senderId;
	...
	function showActionPopup(message) {
		...
		if ((Date.now() - message.timestamp) < MAX_RECALLABLE_TIME && message.senderId === currentUser.id && message
			.status === 'success') {
